...
let showDiscoveredElem = document.querySelector(".show_discovered");
...
showDiscoveredElem.addEventListener("click", (e)=>{toggleUiValue(e)});
...
showDiscoveredTiles = true; // needs to be global for window[] access
...
function drawTiles() {
    ...
    aliveTiles.forEach((tile) => {
        ...
        ctx.rect(tile.worldX, tile.worldY, tile.w, tile.h); // -1 to show spacing between tiles
    });
    ctx.fill();
    ...
    // this is not efficient b/c we have to change fillstyle for every tile
    // would be better sort by color first????
    if(showDiscoveredTiles) {
        discoveredTiles.forEach((tile) => {
            ctx.beginPath();
            ctx.fillStyle = discoveredColors[tile.discoveredTimes];
            ctx.moveTo(tile.worldX, tile.worldY);
            ctx.rect(tile.worldX, tile.worldY, tile.w, tile.h); // -1 to show spacing between tiles
            ctx.fill();
        });
...
function checkTilesNextGen() {
    // set whether each tile will be alive or dead for next frame
    // based on current number of neighbors
        ...
        for (let y = 0; y < tilesPerCol; y++) {
            ...
            if(tile.alive) {
                if(neighborsAlive < 2 || neighborsAlive > 3) { // under 2 die underpopulation, over 3 die overpop
                    ...
                    if(tile.discoveredTimes < discoveredColors.length - 1) {
                        tile.discoveredTimes++;
                    }
                }
